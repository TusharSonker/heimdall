...
def predict_raw(req: PredictRequest):
    ...
    try:
        ...
        logger.error(f"[{req.model_id}] Base10 inference error: {e}")
        raise HTTPException(status_code=500, detail=f"Base10 inference failed: {e}")
    ...
    return PredictResponse(
        ...
        threshold=threshold,
        inference_time_ms=round(elapsed_ms, 2),
        server_note="Base10EncodedNumber path (phe override). Compare with /api/predict.",
    )
